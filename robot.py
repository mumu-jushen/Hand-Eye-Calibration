...
class RobotBase(object):
    ...
    def get_joint_obs(self):
        ...
        ee_pos = p.getLinkState(self.id, self.eef_id, computeForwardKinematics=1)[:2]
...
class Panda(RobotBase):
    def __init_robot__(self):
        ...
        self.eef_id = 7 #9
        self.arm_num_dofs = 7
        # self.arm_rest_poses = [0., -math.pi/4 - 0.2, 0., -3*math.pi/4, 0, math.pi/2 + 0.2, math.pi/4]
        self.arm_rest_poses = [0., -math.pi/4 - 0.2, 0., -3*math.pi/4, 0, math.pi/2 + 0.2, math.pi/2]
        # self.arm_rest_poses = [0., -0.785398163, 0., -2.35619449, 0, 1.57079632679, 0.785398163397]

        print('self.arm_rest_poses', self.arm_rest_poses)
        self.id = p.loadURDF('./urdf/panda2.urdf', self.base_pos, self.base_ori,
